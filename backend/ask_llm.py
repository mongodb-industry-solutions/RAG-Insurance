...
client = MongoClient(mdb_uri, appname=APP_NAME)
...
def vector_search(question):
    ...
    for res in semantic_search_results:
        res.metadata.pop('damageDescriptionEmbedding', None)
        res.metadata.pop('_id', None)
        res.metadata.pop('photoEmbedding', None)
        res.metadata.pop('claimDescriptionEmbedding', None)
